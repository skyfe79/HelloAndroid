 ...
 */
@EActivity(R.layout.activity_origami_ex)
public class OrigamiExActivity extends Activity{
    @ViewById(R.id.grid)
    View mPhotoGrid;
    ...
    @ViewById(R.id.spring_configurator)
    SpringConfiguratorView mSPringConfiguratorView;

    // Create a spring configuration based on Origami values from the Photo Grid example.
    static final SpringConfig ORIGAMI_SPRING_CONFIG = SpringConfig.fromOrigamiTensionAndFriction(40, 7);
    Spring mSpring;
    ...
    public void onAfterViews() {
        ...
        mPhotoGrid.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
    ...
    private void render() {
        ...
        float barPosition = (float) SpringUtil.mapValueFromRangeToRange(value, 0, 1, mFeedbackBar.getHeight(), 0);
        mFeedbackBar.setTranslationY(barPosition);

        // Map the spring to the selected photo scale as it moves into and out of the grid.
        float selectedPhotoScale = (float)SpringUtil.mapValueFromRangeToRange(value, 0, 1, 0.33, 1);
